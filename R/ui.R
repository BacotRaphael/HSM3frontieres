ui <- shiny::fluidPage(
  ...
  shiny::sidebarLayout(
    ...
    shiny::mainPanel(
      ...
      p("Cette application est destinée pour tout le public, pas besoin d’avoir des connaissances en R ou autre langage de programmation pour manipuler l’application."),
      ...
      p("Comme la vérification peut différer d’une mission à l’autre, faut bien choisir le pays pour lequel on collecte les données. Le script va filtrer les données et prendre en compte seulement le pays sélectionné."),
      p("C’est possible d’avoir un cleaning log avec des champs labéliser (questions et réponses) en choisissant l’option",strong("Oui"), "dans", strong("‘Voulez vous labeliser les questions/reponses dans le cleaning log’")),
      ...
      p("En générale, une base de donnée doit avoir une variable clé unique (« uuid »), pas de variable dupliquée et reflète l’outil de collecte de données. Si on rajoute des variables/options on doit aussi modifier le questionnaire KOBO."),
      ...
      p("Une fois l’utilisateur fournit ces inputs, le bouton",strong("RUN checks"), "sera activé et l’utilisateur peut lancer l’opération. Une fois les calculs terminés, le bouton",strong("Download Cleaning LOG")," sera à son tour activé pour télécharger le cleaning LOG en format CSV."),
      ...
      p("Une fois l’utilisateur fournit ces inputs, le bouton", strong("Clean data"), "sera activé et l’utilisateur peut lancer l’opération. Une fois les calculs terminés, le bouton", strong("Download Clean data sera à son tour activé pour télécharger les données nettoyées en format CSV.")),
      ...
      p(strong("Skip Logic list (.csv or .xlsx)")," :Charger le fichier qui définit comment les « SL » doivent être implémenter. A mettre à jour si on modifie la colonne « relevant » du questionnaire"),
      p("Apres l’utilisateur doit sélectionnez les variables d’agrégation dans l’ordre (admin0, admin1, admin2, admin3, admin4 …etc.). Le champs", strong("« Selectionnez les variables d'agrégation dans l'ordre (admin) » apparaitra une fois que les données et le questionnaire sont chargés.")),
      p("Vous avez le choix d’avoir les données agrégées en XML ou labellisées."),
      p("Une fois l’utilisateur fournit ces inputs, le bouton", strong("AGGREGATION"), "sera activé et l’utilisateur peut lancer l’opération. Une fois les calculs terminés, le bouton", strong("Download Aggregated DATA"), "sera à son tour activé pour télécharger les données agrégées en format CSV."),
      ...
      p(strong("RAPPEL : XML valeurs et en-tête, point comme séparateur de groupe, ne pas inclure les groupes dans les en-têtes, pas de variables dupliquées. Si des colonnes/options ont été rajouté, modifiez l’outil de collecte de données.")),
      p("Une fois l’utilisateur fournit ces inputs, le bouton", strong("CONVERTIR"), "sera activé et l’utilisateur peut lancer l’opération. Une fois les calculs terminés, le bouton", strong("Download Converted DATA"), "sera à son tour activé pour télécharger les données convertis en format CSV."),
      ...
      p(strong("RAPPEL : Point comme séparateur de groupe et non pas la barre oblique « / », ne pas inclure les groupes dans les en-têtes, pas de variables dupliquées. Si des colonnes/options ont été rajouté, modifiez l’outil de collecte de données. Kobotoolbox ne te permet pas de charger un questionnaire XLSform si la colonne « name » n’est pas unique ce qui n’est pas le cas pour la colonne « label ». Faites bien attention, pas des en-têtes qui portent les mêmes labels. Si vous changez des colonnes/options, modifiez aussi le questionnaire.")),
      p("Une fois l’utilisateur fournit ces inputs, le bouton", strong("CONVERTIR"), "sera activé et l’utilisateur peut lancer l’opération. Une fois les calculs terminés, le bouton", strong("Download Converted DATA"), "sera à son tour activé pour télécharger les données convertis en format CSV."),
      br()
    )
  )
